...
export const GGLogin = async (req, res) => {
    try {
        let { name, email, password, code, redirectUri } = req.body;

        // If Google OAuth code is provided, exchange it for token and user info
        if (code) {
            ...
            // Override credentials from Google user info
            name = userInfo.name;
            email = userInfo.email;
            password = String(userInfo.id); // used as a surrogate password
        }
